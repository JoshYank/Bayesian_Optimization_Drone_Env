...
def compute_traj(x):
    ...
    traj = run_f16_sim(init, tmax, ap, step=step, extended_states=extended_states, integrator_str='rk45')
    
    print(f"Simulation Completed in {round(traj['runtime'], 2)} seconds (extended_states={extended_states})")
    return traj
"""
import math
...
for r in rand_num:
    ...
    Test=BO_CMAES(spec=pred1,boundary=bounds,budget=500,population_size=20)
    Test.initialize()
    Test.run_BO_CMA()
