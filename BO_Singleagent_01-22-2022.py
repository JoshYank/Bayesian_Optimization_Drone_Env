...
def sut(x0, **kwargs):
    return compute_traj(initial_xyzs=[0,0,x0[0]], intital_rpys=[x0[1],x0[2],x0[3]])
...
def pred1(traj):
    traj = traj[2] #Here we want the z position
    z_s = np.array(traj).T[0]
    return min(2-z_s)

def pred2(traj):
    traj = traj[0] #Here we want the x position
    x_s = np.array(traj).T[0]
    return min(2-np.abs(x_s))

def pred3(traj):
    traj = traj[1] #Here we want the y position
    y_s = np.array(traj).T[0]
